...
# SQL Integration
@st.cache
def insertBLOB(reg, classID, Date, att, name, duration):
    mydb = mysql.connector.connect(host="localhost", database="giraffe", user="root", password="1234")
    my_cursor = mydb.cursor()
    sql_insert_blob_query = "INSERT INTO attendance (reg, ClassID, Date, att, name, duration) VALUES (%s,%s,%s,%s,%s,%s)"
    # Convert data into tuple format
    insert_blob_tuple = (reg, classID, Date, att,name, duration)
    result = my_cursor.execute(sql_insert_blob_query, insert_blob_tuple)
    mydb.commit()
    print("Entry updated successfully in Attendance table")

if(flag==1):
    insertBLOB(reg,classID, today_date, att,name, min)
